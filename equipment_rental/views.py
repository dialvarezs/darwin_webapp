...
def lease_list(request):
    ...
    equipments = Equipment.objects.annotate(
        available=F("stock") - F("stock_leased")
    ...
    context = {
        "logo": "resources/img/logo-camaleon-mini.png",
        "leases": leases,
        "clients": clients,
...
def lease_save(request, lease_id=None):
    if lease_id:  # return
        try:
            ...
            new_payment = (
                request.POST["new_payment"] if request.POST["new_payment"] != "" else 0
            )
                ...
                if has_surcharges:
                    for epk, d, t, v in zip(
                        ...
                        if d != "" and d != "0":
                            ...
                            sm = StockModification(
                                equipment=le.equipment,
                                quantity=-1 * int(d),
                                description=desc,
                                record=datetime.now(),
                            )
        ...
        try:
            ...
            notes = request.POST["notes"]
            equipments = [
                {"e": Equipment.objects.get(pk=e), "n": n}
                for e, n in zip(
                    request.POST.getlist("equipments_select[]"),
                    request.POST.getlist("equipments_quantity[]"),
                )
            ]
        except KeyError:
            pass
