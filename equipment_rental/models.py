...
class Equipment(models.Model):
    ...
    stock_leased = models.PositiveIntegerField(
        default=0, verbose_name="cantidad arrendada"
    )
...
class StockModification(models.Model):
    equipment = models.ForeignKey(
        Equipment, on_delete=models.CASCADE, verbose_name="equipamiento"
    )
...
class Lease(models.Model):
    ...
    return_record = models.DateTimeField(
        blank=True, null=True, verbose_name="registro de entrega"
    )
    ...
    notes = models.CharField(
        max_length=256, blank=True, null=True, verbose_name="notas"
    )
    equipments = models.ManyToManyField(
        Equipment, through="LeaseEquipments", verbose_name="equipamientos"
    )
    ...
    def __str_equipments__(self):
        return str(
            ", ".join(
                "{} ({})".format(e.equipment, e.quantity)
                for e in self.leaseequipments_set.all()
            )
        )
...
class Surcharge(models.Model):
    lease = models.ForeignKey(Lease, on_delete=models.CASCADE, verbose_name="arriendo")
    lease_equipment = models.ForeignKey(
        LeaseEquipments,
