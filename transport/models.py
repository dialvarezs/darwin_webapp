...
class Bus(models.Model):
    company = models.ForeignKey(
        BusCompany, on_delete=models.PROTECT, verbose_name="empresa"
    )
...
class Stretch(models.Model):
    ...
    def __str_destinations__(self):
        return " - ".join(
            [
                x.short_name
                for x in self.destinations.all().order_by(
                    "stretchdestinations__position"
                )
            ]
        )
...
class StretchDestinations(models.Model):
    ...
    destination = models.ForeignKey(
        Destination, on_delete=models.CASCADE, verbose_name="destino"
    )
...
class Passenger(models.Model):
    ...
    id_type = models.ForeignKey(
        IDType, on_delete=models.PROTECT, verbose_name="tipo de identificación"
    )
    names = models.CharField(max_length=32, verbose_name="nombres")
    surnames = models.CharField(max_length=32, verbose_name="apellidos")
    ...
    def __str__(self):
        return "{} {}: {}, {}".format(
            self.id_type, self.id_string, self.surnames, self.names
        )
...
class Group(models.Model):
    ...
    company = models.ForeignKey(
        Company, on_delete=models.PROTECT, verbose_name="empresa"
    )
...
class Travel(models.Model):
    ...
    bus = models.ForeignKey(
        Bus, on_delete=models.PROTECT, blank=True, null=True, verbose_name="bus"
    )
    driver = models.ForeignKey(
        Driver, on_delete=models.SET_NULL, blank=True, null=True, verbose_name="chofer"
    )
    additional_driver = models.ForeignKey(
        Driver,
        on_delete=models.SET_NULL,
        related_name="%(app_label)s_aditional_%(class)s_related",
        blank=True,
        null=True,
        verbose_name="chofer adicional",
    )
    stretch = models.ForeignKey(
        Stretch, on_delete=models.PROTECT, blank=True, null=True, verbose_name="tramo"
    )
