...
from .models import Bus, BusCompany, Company, Driver, Group, Stretch, Travel
from .pdf_print import DocumentBuilder


@login_required
def travel_list(request):
    travels = Travel.objects.all().select_related(
        "group", "bus", "bus__company", "stretch", "driver"
    )
    ...
    if "company" in request.GET and request.GET["company"] != "0":
        ...
        filters.append(
            "Empresa (Turismo): "
            + Company.objects.get(pk=request.GET["company"]).__str__()
        )
    ...
    if "buscompany" in request.GET and request.GET["buscompany"] != "0":
        ...
        filters.append(
            "Empresa (Buses): "
            + BusCompany.objects.get(pk=request.GET["buscompany"]).__str__()
        )

    if "driver" in request.GET and request.GET["driver"] != "0":
        travels = travels.filter(
            Q(driver=request.GET["driver"]) | Q(additional_driver=request.GET["driver"])
        )
        filters.append(
            "Chofer: " + Driver.objects.get(pk=request.GET["driver"]).__str__()
        )
    ...
    if date_from and date_to:
        ...
        filters.append(
            "Fechas: {} - {}".format(
                datetime.strptime(date_from, "%Y-%m-%d").strftime("%d/%d/%y"),
    ...
    travels = __pagination(travels, 40, request.GET.get("page"))

    context = {
        "logo": "resources/img/logo-darwin-mini.png",
        "travels": travels,
        "groups": groups,
        "busses": busses,
...
def travel_pdf(request):
    ...
    if "company" in request.GET and request.GET["company"] != "0":
        ...
        info_raw += (
            "Empresa (Turismo):"
            + Company.objects.get(pk=request.GET["company"]).short_name,
        )
    else:
    ...
    if "driver" in request.GET and request.GET["driver"] != "0":
        ...
        info_raw += (
            "Conductor:" + Driver.objects.get(pk=request.GET["driver"]).__str__(),
        )
    else:
